...
- Headline claim: 23 grouped predictions, matching Paper 2.
...
| F0 | Triality-breaking parameter | `epsilon_0^2 = pi/432` | Derived bridge, high priority | Replace dimensional counting with an explicit transition amplitude or operator trace |
| M1 | Charm mass | `m_c = epsilon_0^2 m_t` | Derived bridge | Derive the single up-sector channel factor |
| M2 | Strange mass | `m_s = 3 epsilon_0^2 m_b` | Derived bridge | Derive the three color-channel multiplicity inside the Yukawa operator |
| M3 | Muon mass | `m_mu = 8 epsilon_0^2 m_tau` | Derived bridge | Derive why color-neutral leptons sample all 8 octonionic directions |
...
| M9 | Up mass | `m_u = (1/4) m_c^2/m_t` | Ansatz / open bridge | Derive `|A/C|^2 = 1/4` from NNI operator geometry |
| M10 | Down mass | `m_d = (9/4) m_s^2/m_b` | Ansatz / open bridge | Derive `N_c^2` enhancement in the first-generation NNI ratio |
| M11 | Electron mass | `m_e = (1/4 pi) m_mu^2/m_tau` | Ansatz / open bridge | Derive the `1/pi` lepton coset average and check Koide correction |
...
| N2 | Neutrino splitting ratio | `Delta m21^2/Delta m31^2 = 4 epsilon_0^2` | Open bridge | Construct the broken-Z3 seesaw matrix and diagonalize it |
| N3 | PMNS reactor angle | `sin^2(theta13) = 3 epsilon_0^2` | Open bridge | Same broken-Z3 seesaw derivation as N2 |
| N4 | PMNS solar angle | `sin^2(theta12) = 1/(3 + sqrt(7) epsilon_0)` | Open bridge | Derive quark-lepton complementarity from the shared triality operator |
| N5 | PMNS atmospheric angle | `sin^2(theta23) = 4/7` | Open bridge / future sensitive | Derive the `4 of 7` direction count dynamically and track octant data |
...
1. Derive `epsilon_0^2 = pi/432` as a transition amplitude over `A` and `J3(O)`.
2. Replace the `compute/flavour_derivation.py` bridge matrices with explicit CHO Yukawa operators whose diagonalization gives the CKM and first-generation NNI factors.
3. Construct the broken-`Z3` seesaw matrix and derive PMNS corrections in one calculation.
