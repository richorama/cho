...
- `compute/epsilon_cubic_discriminant.py`, `compute/epsilon_heat_kernel.py`, `compute/epsilon_state_count.py`, `compute/epsilon_product_space.py`, `compute/epsilon_weyl_isomorphism.py` — five triangulating experiments on `epsilon0^2 = pi/432` (F0). They rule out the heat-kernel origin of the `pi` and the cubic-discriminant origin of the `27`, DERIVE `16 = dim OP^2 = F4/Spin(9)` as the rank-one idempotent manifold of `J3(O)`, stratify `27 = 1 + 16 + 10`, and DISCHARGE the residual isomorphism `A_Weyl ~= T(OP^2)` by showing both are the unique 16-dim real Spin(9) spinor `Delta_9` (`f4 = Der(J3(O))` and its `spin(9)` stabiliser built from the octonion table; isotropy and octonionic-`Cl(9)` modules both irreducible of real type).
