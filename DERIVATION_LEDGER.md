...
|---|---|---|---|---|
| G1 | Number of generations | `N_gen = 3` | Theorem (rep counting) conditional on A2-A3 bridge (open); independent SPECTRAL route now available | `three_generations_nogo_audit.py` confirms the SOUND part (D4 triality = S3 gives exactly three inequivalent 8-dim reps 8v, 8s, 8c) but shows the physical bridge A3 faces two Distler-Garibaldi-style obstructions: triality mixes a vector (8v) with two spinors (8s, 8c), and the two spinors are opposite-chirality (mirror) partners. Until A3 selects only fermionic reps and avoids the mirror pairing, the '3 reps = 3 chiral generations' step is CONJECTURE-level, not theorem. NEW: `jordan_eigenvalue_generations.py` (Lever A) supplies a SECOND, obstruction-free reason for three -- it is the DEGREE of the J3(O) cubic norm (= rank of the algebra = three orthogonal primitive idempotents), a spectral fact that does NOT use representation theory and so is immune to the vector/spinor + mirror obstruction. This explains the COUNT but not yet the spectrum (a flavour-diagonal element returns only its seeded diagonal; physical content remains an open Yukawa-map target) |
...
| STAT1 | 25-row audit table | Median error and pull summary | Descriptive diagnostic / covariance closed / Bayes factor computed | `covariance_gof.py` builds the full covariance with a shared-`eps0` common mode: the 22 independent rows collapse to `N_eff ~ 10` effective observables, and the correlated reduced `chi^2 ~ 1.8` against `N_eff` (`p ~ 0.06`) -- consistent but borderline, superseding the diagonal-with-floor figure and the raw row count. `bayesian_evidence.py` goes further: charged the FULL Occam price for all ~83 bits of chosen prefactors, the model-comparison Bayes factor FAVOURS the O(1)-numerology null (`ln B ~ -21` at prior width F=3) -- the honest skeptic's strongest point. It flips to favour CHO only when ~31 of those 83 bits are DERIVED (cost -> 0); deriving prefactors is therefore the single decisive lever |
...
- `compute/epsilon_cubic_discriminant.py`, `compute/epsilon_heat_kernel.py`, `compute/epsilon_state_count.py`, `compute/epsilon_product_space.py`, `compute/epsilon_weyl_isomorphism.py`, `compute/epsilon_spin9_embedding.py`, `compute/epsilon_rank_one_kernel.py` — seven triangulating experiments on `epsilon0^2 = pi/432` (F0). They rule out the heat-kernel origin of the `pi` and the cubic-discriminant origin of the `27`, DERIVE `16 = dim OP^2 = F4/Spin(9)` as the rank-one idempotent manifold of `J3(O)`, stratify `27 = 1 + 16 + 10`, DISCHARGE the isomorphism `A_Weyl ~= T(OP^2)` (both are the unique 16-dim real Spin(9) spinor `Delta_9`), close the gauge-vs-flavour Spin(9) seam by recovering the flavour octonionic `Cl(9)` (Casimir `1,9,126`; bivectors = flavour `so(9)`; `O(16)`-conjugate), and reframe R1 (rank-one kernel = primitive idempotent = pure single-generation vacuum, dual to `N_gen=3`).
...
- `compute/bayesian_evidence.py` — model-comparison Bayes factor CHO vs an O(1)-numerology null on the independent set, correlation- and parameter-penalised. Honest negative at full parameter cost (favours the null until ~31 of 83 prefactor bits are derived); makes the derivation program the explicit lever.
