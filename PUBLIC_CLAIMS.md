...
| SM-constant physics claim | The theorem-level math is durable, but the claim that CHO derives the Standard Model constants is on probation | The only active internal route is a derived `F4`-breaking action whose flux gives `pi/432` and whose spectrum gives the seed; otherwise demote to structured algebraic numerology | CHO is already a theory of nature; the constants are dynamically derived |
