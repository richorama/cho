...
- [compute/claim_status_report.py](compute/claim_status_report.py) — grouped claim-status report generated from the semantic contract registry
...
- [compute/epsilon_free_mixing_web.py](compute/epsilon_free_mixing_web.py) — **the `π/432` knob cancels (eps0-free web).** Dividing the three measured mixing relations above pairwise (`|V_us|²=7ε₀²`, `sin²θ₁₃=3ε₀²`, `Δm²₂₁/Δm²₃₁=4ε₀²`) cancels `ε₀=√(π/432)` identically, leaving **parameter-free** exact-rational cross-sector relations from the same Fano split: `sin²θ₁₃=(3/7)|V_us|²` (the reactor angle as `3/7` of the squared Cabibbo angle — the *through-vacuum* twin of the `4/7` octant bet Q2), `Δm²₂₁/Δm²₃₁=(4/3)sin²θ₁₃`, and the completeness sum rule `sin²θ₁₃+Δm²₂₁/Δm²₃₁=|V_us|²` (`3ε₀²+4ε₀²=7ε₀²`). All hold at `<~1.2σ` with current data (the purely-leptonic `4/3` at `+0.16σ`) and are testable **now**, not after DUNE. Diagnostic only — tests the Fano counting *given* its channel assignments (the open `C1/N2/N3/N5` bridges); promotes no row and moves no Bayes credit
- [compute/lepton_yukawa_action.py](compute/lepton_yukawa_action.py) — **ONE action → ONE charged-lepton Yukawa, end-to-end (Item 2).** The two-level Bloch sphere `S²` of [foundations/02_action.md](foundations/02_action.md) supplies BOTH lepton numbers: its hemisphere solid angle `2π` is the Berry `θ=π` (Bargmann great-circle holonomy, `9e-16`), and its TOTAL solid angle `4π` is the `SU(2)` invariant-average (Schur) normalization — so the shape factor `k_l=1/(4π)` is FORCED as `1/(total solid angle)` of the SAME sphere, upgrading it from "identified" to "forced". Reusing the derived Fock trace `8` (M3) and the rank-one "lift exactly one level" cascade, it assembles the single Hermitian `Y=m_τ·diag(1, 8ε₀², k_l(8ε₀²)²)`: `τ` exact, `μ/τ` `−2.2%`, `e/τ` `−6.3%` (the known M11 outlier, reported — only `μ` is asserted). Open: WHY leptons take the continuous-sphere average while quarks take discrete weak-isospin projections (`¼`, `9/4`, no `π`); the trilinear from the CHO equations of motion; the `~6%` `m_e` residual; F0 NOT promoted
...
- [compute/epsilon_measure_witness.py](compute/epsilon_measure_witness.py) — F0 seam isolator: pins the live H4 question explicitly as a theorem seam (not a tuning knob), separating what is already geometric from what still needs derivation credit
- [compute/epsilon_measure_schur.py](compute/epsilon_measure_schur.py) — Schur closure of the measure-normalization half of H4. It proves the `1/16` and `1/27` weights are forced by irreducibility: `Spin(9)` on `Delta_9` has commutant `1` (`-> 1/16`), `F4=Der(J3(O))` on `27` is reducible (`27=1+26`, commutant `2`, not flat), and the full cubic-norm `E6` action is irreducible (commutant `1`, `-> 1/27`). So the normalization is theorem-level, while the remaining open seam is the product phase-space identification `Delta_9 × J3(O)`
- [compute/epsilon_phase_space_product.py](compute/epsilon_phase_space_product.py) — product-phase-space witness for the post-Schur seam. It makes one explicit assumption, sector independence (commuting Spin(9)/E6 actions with minimal multiplicity), and shows that under this assumption the canonical carrier is `Delta_9 × J3(O)` and the factorized Reynolds average is exact: `(I_16/16) × (I_27/27) = I_432/432`, hence `pi/432`. This narrows the live F0 question to deriving sector-independence from the action/one-operator dynamics, rather than re-litigating normalization.
- [compute/epsilon_assumption_p_gate.py](compute/epsilon_assumption_p_gate.py) — action/operator gate for that same seam. On the current one-operator scaffold, the epsilon bridge operator has exact `16×27` operator-Schmidt rank `1`, reconstructs as a primitive product `A⊗B` to machine precision, and yields normalized trace `pi/432`. A stress test with an added independent product term raises Schmidt rank to `2`, so the gate is sensitive to genuine cross-sector mixing. This is strong evidence for Assumption P at operator level, while the open step remains deriving that separable primitive structure from CHO action dynamics rather than inserting it.
- [compute/epsilon_product_irreducible.py](compute/epsilon_product_irreducible.py) — removes the circular part of the gate above and collapses Assumption P from three clauses to one. It proves `Delta_9 × J3(O)` is irreducible under the factor-wise product group `Spin(9) × E6` (computed by partial-trace Reynolds averaging, `<P>_{G1×G2} = (1/16) I_16 × <Tr_1 P>_{E6}`, with no `432×432` element ever formed), so by Schur EVERY rank-one spurion averages to `I_432/432`. A maximally entangled spurion (operator-Schmidt rank `16`) and a separable one (rank `1`) give the SAME flat average to `5e-18` — so flatness does NOT depend on separability or minimal multiplicity, which is exactly the input the gate had assumed. An `F4` control (leaves `27` reducible, residual `1e-2`) confirms the factor-wise `E6` is necessary. Net: Assumption P reduces to the single clause "factor-wise `Spin(9) × E6` invariance"; F0 is NOT promoted, because deriving that product arena from the CHO action remains the live seam.
- [compute/epsilon_symplectic_volume.py](compute/epsilon_symplectic_volume.py) — attacks that surviving "why a product" clause with a different lens, geometric quantization. It recomputes `16` and `27` from scratch (Weyl dimension formula) as the Bohr–Sommerfeld / Borel–Weil state counts of single coadjoint orbits — the `Spin(9)` spinor orbit and the `E6` minimal orbit — and verifies on small product root systems (`A1×A1→4`, `A2×A1→6`, `A2×A2→9`) that the quantization is MULTIPLICATIVE over a direct-product group. So `432 = 16 × 27` is the Liouville volume of one PRODUCT orbit as a theorem of the orbit method: "the arena is a product" follows from "the symmetry is a product group". The bare `pi` is recovered as the symplectic half-flux of the minimal transition orbit `CP^1 = S^2` (reusing the great-circle Berry phase), giving `pi/432`. Honest caveat: `OP^2 = F4/Spin(9)` is itself non-symplectic, so the carriers are the orbits in `so(9)^*`, `e6^*`, not `OP^2`; F0 is NOT promoted, but the seam sharpens to "which two coadjoint orbits does the action quantize", now with a positive geometric mechanism for the product.
- [compute/epsilon_factor_forcedness.py](compute/epsilon_factor_forcedness.py) — the `look_elsewhere` discipline applied to the factorization itself. Among all ten ways to write `432 = a × b`, `16 × 27` is the UNIQUE split whose both factors are independently-derived carriers (`16 = Delta_9 = dim OP^2`, `27 = dim J3(O)`), strictly out-scoring every rival (`8 = dim O`, `4 = dim H`, `36 = dim so(9)`, … each paired with a structureless partner). A single-group no-go scan recomputes the fundamental-rep dimensions of `su(n)`, `so(n)`, and the exceptionals `G2, F4, E6, E7, E8` from their root systems and finds `432` among NONE of them, while it IS the `Spin(9) × E6` (spinor)×(minimal) bifundamental — so a product realization is forced. This removes the factorization freedom; honest scope: it ranks pre-derived structure and is a minimal/fundamental-rep statement, not an absolute no-go.
- [compute/higgs_quartic_geometry.py](compute/higgs_quartic_geometry.py) — **the same forcedness question for the sibling constant, the Higgs quartic `λ = π/24`.** Pure exact arithmetic: `432 = 18 × 24`, so `λ = π/24 = 18·ε₀²` — the Higgs quartic and the flavour seed are **not** two independent transcendentals; they **share one Berry `π`** and differ by a pure integer (given `ε₀² = π/432` carries the holonomy `π`, the `π` in `λ` is forced to be the *same* one — one fewer independent transcendental, not the cited independent "D4 root geometry" assumption). Honest negative: the integer `24` is **not uniquely forced** — it has ≥3 framework origins (`|roots(D4)| = 28−4`, off-diagonal `J3(O) = 27−3 = 3·dim O`, `|2T| = 2|A4|`), so exactly like `432` no `F4`-breaking action selects it: the same FORM-not-CONTENT wall on a different observable. Registered `diagnostic` (S3/F0); promotes no row, moves no Bayes credit
- [compute/epsilon_action_stationary.py](compute/epsilon_action_stationary.py) — variational strengthening of that evidence. In the normalized link-action class (`O>=0`, `Tr(O)=pi`), the primitive kernel `O=pi|tau><tau|` is the unique global maximizer by Cauchy equality, and the current scaffold operator saturates it exactly. Random PSD/separable stress scans stay strictly below the maximum. This moves the seam from "is primitive separable plausible?" to "derive the physical transition ray and admissible kernel class from full CHO dynamics."
- [compute/f0_action_ray_gate.py](compute/f0_action_ray_gate.py) — direct action-level ray derivation gate in the current scaffold. It solves the stationarity equation `H|tau>=lambda_max|tau>` and confirms dynamic convergence (imaginary-time ascent) to one unique transition ray representative.
- [compute/f0_action_kernel_dynamics_gate.py](compute/f0_action_kernel_dynamics_gate.py) — action-evolution derivation gate for admissible kernels. It checks closure under Liouville flow (`rho -> U rho U^dagger`), convex coarse-graining, and `theta=pi` normalization, yielding `O>=0` and `Tr(O)=pi` in the effective dynamics.
- [compute/f0_kernel_class_gate.py](compute/f0_kernel_class_gate.py) — admissible-class checkpoint for the second F0 seam. It verifies closure consistency of the kernel class under the current assumptions (pure-ray orbit, convex mixing, basis-indifferent averaging, and `theta=pi` normalization), yielding `O>=0`, `Tr(O)=pi` on the 16×27 bridge. The residual remains deriving those assumptions directly from full CHO action dynamics.
- [compute/f0_vacuum_orbit_gate.py](compute/f0_vacuum_orbit_gate.py) — orbit-selection checkpoint for the ray representative. Once the vacuum is fixed, the Fano transition class collapses to a single stabilizer orbit, so the ray is selected up to residual gauge rather than by hand. The remaining open seam is the CHO-action derivation of the physical representative itself.
- [compute/f0_transition_ray_gate.py](compute/f0_transition_ray_gate.py) — consistency gate for the physical transition ray. It checks that the vacuum orbit, the action-selected great circle, and the unique 16×27 trace space all point to the same representative. The consistency is useful, but it still leaves the direct CHO derivation of the ray open.
- [compute/f0_direction_gate.py](compute/f0_direction_gate.py) — governance guardrail so momentum stays useful. It reads live F0 contracts plus the Bayes ladder and prints a continue-vs-pivot recommendation: continue only on tasks that retire a named live F0 bridge; otherwise pivot to falsification pressure or submission packaging. This is intentionally anti-theory-for-theory.
...
- [compute/rg_scale_derivation.py](compute/rg_scale_derivation.py) — **can the electroweak matching scale be DERIVED? No — it is inverse-fit (Item 3, KILL branch).** CHO's two electroweak boundaries (`α_em⁻¹=128π/3` and `sin²θ_W=1/4`) fix BOTH gauge couplings (`α₁⁻¹=96π/5`, `α₂⁻¹=32π/3`) at one would-be scale, but one-loop running reaches them at `μ≈12 GeV` vs `μ≈2.2×10⁵ GeV` — a `1.8×10⁴` discrepancy, so no single matching scale exists. The lone inverse-matched `sin²=1/4` scale is `M_P/3^32.5` (a non-integer power of three) and no independently-derived CHO scale (`v`, `M_W`, seesaw, `M_P`) lands at `1/4`. The matching seam is now falsified as a single derived number; S4/S5 stay Open bridge with a sharper reason, no row promoted
- [compute/neutrino_floor_resolution.py](compute/neutrino_floor_resolution.py) — **is the `m_ν3` floor deficit a 4.6σ falsification? No — it is a ~1.2σ undershoot once the tree-level seesaw carries its own theory error (Item 4, RESOLVE branch).** The `4.6σ` divides the `1.2 meV` deficit (`m_ν3=48.9` vs floor `50.1 meV`) by the floor's experimental error alone, assigning ZERO theory error to a tree-level seesaw. Calibrating its two `O(1)`-uncertain inputs from their sister rows (`M_R` from `M_W=M_P/3³⁶`, `+1.2%`; `y_ν3=1` from `y_t=1`, `+0.8%`, squared) gives a `~2.0%` theory error (`±1.0 meV`), so the honest significance is `~1.2σ` — consistent. N1 is **demoted** from a precision claim to order-1 consistency; the central value is stated plainly as still just below the physical floor; the obligation is to derive the `O(1)` normalization, no row promoted
- [compute/per_row_theory_error.py](compute/per_row_theory_error.py) — **Item 6: tighten the theory error per row as bridges close, never with one global floor.** Each row's theory error is keyed to its weakest-bridge derivation status (the frozen `DERIVED`/`GEOMETRIC`/`CHOSEN` taxonomy plus the open-residual ledger flags), with the tier magnitudes fixed from independent sister-row figures (not tuned) and independently cross-checked by the `eps0`-ladder's own `1.23%` RMS. Two-sided result: the lone fully `eps0`-independent exact theorem `sin²θ₂₃=4/7` **earns a precision test and passes** (exp pull `−0.02`) — non-fudgeable evidence; but tightening the DERIVED-prefactor ladder rows `m_s`, `m_μ` to sub-percent is **falsified at 2.4–2.8σ** (they are `~1.5–2%` relations, not precision), so the geometric tier honestly stays at `~1.5%` until the ladder normalization is derived; the continuum rows `α⁻¹`, `sin²θ_W` are derivation-limited and **excluded, not inflated**. A sensitivity scan (halving every error → `p~0.09`) shows the widths are real, not padding; a GoF refinement only — no row promoted, scoreboard untouched
- [compute/theta23_octant_prediction.py](compute/theta23_octant_prediction.py) — **Item 7: the single sharpest falsifiable claim staked on an unmeasured quantity — `sin²θ₂₃ = 4/7 = 0.5714` (`θ₂₃ ≈ 49.1°`, UPPER octant).** Of every CHO prediction this one is chosen because it is the ONLY mixing number that is an exact rational AND `eps0`-INDEPENDENT: the module verifies `d sin²θ₂₃/d eps0 = 0` exactly under an `eps0 ± 20%` scan while the control `sin²θ₁₃ = 3 eps0²` moves — so it sits clear of the open `π/432` seam that hinges the Bayes factor, and it already passed the Item-6 precision test (pull `−0.02`). The octant is the **Fano discriminator**: the lower-octant mirror is exactly `cos²θ₂₃ = 3/7`, and the same partition that fixes the value — `4` Fano lines avoiding the vacuum `e₇` vs `3` through it — fixes the octant (`4 > 3` *is* "upper"), so an octant measurement directly tests which Fano class controls `θ₂₃`. The octant is currently **unresolved** (T2K/NOvA tension), making this a genuine pre-registered bet, not a postdiction; **KILL** = a stable lower-octant resolution (`sin²θ₂₃ < ½`) or an upper value pinned far from `4/7`; decisive reach **DUNE / Hyper-Kamiokande**. A forward test only — the Fano-count→angle map stays a derived bridge (N5), the frozen registry (Q2) stays authoritative, and no row is promoted (Bayes factor untouched)
