...
import f4_breaking_cho_lindbladian_gate
...
ARTIFACTS = [
    ...
    ("f4_breaking_cho_lindbladian_gate",
     "F0 CHO Lindbladian gate (EXPLORATORY): tests whether the depolarizing-toward-P relaxation channel assumed by the vacuum-relaxation gate is generated by a concrete dynamics. The GKSL/Lindblad dissipator L(rho)=gamma(Tr(rho)P-rho), jump operators L_k=sqrt(gamma)|p><e_k| (amplitude damping into the vacuum ray), has finite-time propagator exp(tL)(rho)=e^{-gamma t}rho+(1-e^{-gamma t})P=C_{r(t)} with r(t)=1-exp(-gamma t), verified on a faithful two-level representation (|<p|q>|^2=Tr(P o Q)=pi/432) using an independent scaling-and-squaring matrix exponential (no SciPy). The generator has a UNIQUE steady state P (the vacuum primitive idempotent), spectral gap gamma=1/tau, and CPTP propagator (Choi PSD); feeding r(Delta_t) into the vacuum-relaxation conditionals reproduces mean d for every interval and the memoryless Born-Markov limit. Controls fail: unitary-only does not relax (degenerate manifold, zero gap), wrong-target damping relaxes to Q (mean 1), dephasing-only relaxes to a mixed non-vacuum state. This reduces 'why the relaxation channel' to 'why this jump rate gamma and vacuum-damping jumps from CHO dynamics'; deriving gamma, the jump operators, the probe interval, and pi/432 from the F4-breaking action remains open, and no Bayes credit moves.",
     f4_breaking_cho_lindbladian_gate.main),
