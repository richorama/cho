...
Ten Predictions with Zero Free Parameters}
...
\begin{abstract}
Working within the framework where the Standard Model arises from the 
algebra $\Alg = \Comp\otimes\Quat\otimes\Oct$ equipped with an information-theoretic 
action on a causal lattice, we derive ten electroweak and flavour observables 
...
(tribimaximal mixing). The proton is predicted stable (no GUT gauge bosons 
exist). All predictions agree with experiment at the 0.1--13\% level. No 
...
We work within the framework where the physics algebra is 
...
second generations) remain undetermined at present. Their prediction requires 
computing the full triality-breaking potential on the $\Alg$ manifold --- 
the hardest unsolved problem in the framework.
