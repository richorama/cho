...
Thirteen Predictions with Zero Free Parameters}
...
action on a causal lattice, we derive thirteen electroweak and flavour observables 
...
All predictions agree with experiment at the 0.1--13\% level. No 
...
corrections to tree-level relations.}
...
No parameter is adjusted; no fit is performed. The discrepancies (0.1--3\%) 
...
second generations) are now partially determined. As shown in 
Section~\ref{sec:mass_relations}, the second-generation masses follow from 
a single triality-breaking parameter $\varepsilon_0^2 = m_c/m_t$ via 
multiplicity factors $N_c = 3$ and $\dim(\Oct) = 8$. The derivation of 
$\varepsilon_0^2 \approx 1/136$ from the coset potential on 
$G_2/\SU(3) \cong S^6$ remains the principal open problem.
