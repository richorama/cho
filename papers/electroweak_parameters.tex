...
Twenty-One Predictions with Zero Free Parameters}
...
action on a causal lattice, we derive twenty-one electroweak and flavour observables 
...
The CKM magnitudes $|V_{us}| = \sqrt{7}\,\varepsilon_0$ and 
$|V_{cb}| = \varepsilon_0/2$ follow from the same parameter with 
octonionic and Weinberg-angle factors (errors 0.6\% and 1.0\%).
...
The PMNS mixing follows from $Z_3$ triality symmetry at the see-saw scale, 
giving $\sin^2\theta_{12} = 1/3$ and $\sin^2\theta_{23} = 1/2$ to 6--9\%.
...
geometry, and 
(ii)~a formal proof that the CKM angles inherit $\varepsilon_0$ with the 
specific factors $7$, $1/4$, and $\tan(\pi/8)$.
