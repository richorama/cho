...
Eighteen Predictions with Zero Free Parameters}
...
action on a causal lattice, we derive eighteen electroweak and flavour observables 
...
$\MR = \MP/3^9$. The CP-violating Jarlskog invariant 
$J = \frac{1}{6\sqrt{6}} \prod (\text{mass ratios}) = 3.01\times 10^{-5}$ 
follows from the Nearest-Neighbour Interaction texture (triality selection rule) 
and the Fano plane overlap angle $\delta = \arccos(1/3)$. The neutrino mixing 
angles $\sin^2\theta_{12} = 1/3$ and $\sin^2\theta_{23} = 1/2$ follow from 
the $Z_3$ triality symmetry of the Majorana mass matrix at the see-saw scale 
(tribimaximal mixing). We further derive three inter-sector mass relations: 
$m_s m_t/(m_b m_c) = 3$ and $m_\mu m_t/( m_\tau m_c) = 8$, with factors 
$N_c = 3$ and $\dim(\Oct) = 8$ from the algebraic structure --- thereby 
providing a first-principles derivation of the Georgi--Jarlskog factor.
The third-generation masses $m_\tau = \sqrt{2}\,\varepsilon_0^2 m_t$ and 
$m_b = (7/3)\,m_\tau$ follow from Higgs normalization and 
$\dim(\mathrm{Im}\,\Oct)/N_c$. The first-generation masses are reproduced 
via the NNI constraint $m_1 m_3/m_2^2 = |A/C|^2$ with factors 
$1/4$, $9/4$, $1/(4\pi)$ related to $\sin^2\theta_W$.
All predictions agree with experiment at the 0.1--8\% level. No 
...
this algebra requires exactly three fermion generations.
...
\item Solar neutrino mixing: $\sin^2\theta_{12} = 1/3$
\item Atmospheric neutrino mixing: $\sin^2\theta_{23} = 1/2$
...
\theta_{13} = 0\,.
\end{equation}
This gives $\theta_{12} = 35.3^\circ$ (exp: $33.4^\circ$, $6\%$ error) and 
$\theta_{23} = 45^\circ$ (exp: $49.2^\circ$, $9\%$ error). The reactor angle 
$\theta_{13} = 8.5^\circ$ requires sub-leading corrections from the charged 
lepton NNI sector; the natural scale is 
$\theta_{13}\sim\theta_C/\sqrt{2}\approx 9^\circ$, in good agreement.
...
$\sin^2\theta_{12}^{\text{PMNS}}$ & $Z_3$ of $\MR$ & $1/3$ & 0.307 & 9\% \\
$\sin^2\theta_{23}^{\text{PMNS}}$ & $Z_3$ of $\MR$ & $1/2$ & 0.572 & 13\% \\
...
experimental $1\sigma$ uncertainty).
...
\emph{Georgi--Jarlskog factor}~\cite{Georgi:1979df}. In SU(5) GUTs, this 
...
charged fermion masses} below $m_t$ are now predicted from a single input 
($m_t$) plus pure algebraic factors---no free parameters.
...
The CKM matrix is now largely determined: the Jarlskog invariant is predicted 
to 2.3\%, $|V_{us}|$ to 7\%, and the texture zero $M_{13} = 0$ is a sharp 
structural prediction. The PMNS mixing follows from 
$Z_3$ triality symmetry at the see-saw scale, giving $\sin^2\theta_{12} = 1/3$ 
and $\sin^2\theta_{23} = 1/2$ to 6--9\%.

The principal remaining discrepancy is $|V_{cb}|$ (predicted 40\% too large in 
the pure Fritzsch texture). Allowing a non-zero diagonal entry $D \neq 0$ 
(sub-leading NNI) can reduce this to $\sim 10\%$, but at the cost of degrading 
the Jarlskog prediction. We interpret this as a genuine next-order correction: 
the leading-order (Fritzsch) result captures CP violation exactly, while the 
mixing magnitudes require sub-leading contributions proportional to higher 
powers of the triality-breaking parameter.
...
(i)~a rigorous derivation of the first-generation factors from the coset 
geometry, and 
(ii)~the correction to $|V_{cb}|$ from sub-leading NNI terms.
