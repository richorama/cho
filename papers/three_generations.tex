...
A third companion paper~\cite{Paper3} proposes that the same algebraic structure 
accounts for the cosmological-constant scale ($\Lambda^{1/4} = 2.31$~meV 
from the same few-input framework), with the remaining free-energy bridge 
obligations tracked separately.
...
This addresses the generation-count puzzle: the number 3 is not treated as an empirical accident 
